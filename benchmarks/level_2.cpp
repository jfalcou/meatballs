...
int main() {
  ...
  for (auto n : { 64, 256, 1024, 4096, 8192, 16384 }) {
    ...
    auto a = create_random_matrix<float>(n, n);
...

    {
      ...
      bench.run("cblas", [&] { cblas_sgemv(CblasRowMajor, CblasNoTrans, n, n, alpha, a.data(), n, x.data(), 1, beta, y_.data(), 1); });
...

    {
      ...
      bench.run("cblas", [&] { cblas_sgemv(CblasRowMajor, CblasTrans, n, n, alpha, a.data(), n, x.data(), 1, beta, y_.data(), 1); });
