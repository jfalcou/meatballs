...
void ssymv_l_kernel_1x4(std::size_t from, std::size_t to,         //
                        std::span<float> a0, std::span<float> a1, //
                        std::span<float> a2, std::span<float> a3, //
                        std::span<float> x, std::span<float> y,   //
                        float* w0, float* w1) {
...
void ssymv_l_kernel_8x1(std::size_t to,                         //
                        std::span<float> a0,                    //
                        std::span<float> x, std::span<float> y, //
                        float* w0, float* w1) {
...
void ssymv_l(float alpha, std::span<float> a, std::span<float> x, float beta,
  ...
  for (auto j = 0; j < n; j += 4) {
    ...
    auto a0 = get_row(m, a, j + 0);
    auto a1 = get_row(m, a, j + 1);
    auto a2 = get_row(m, a, j + 2);
    auto a3 = get_row(m, a, j + 3);

    auto b = (j / 8) * 8;

    // full block (4n x 4 and 4 x 4)
    if (b)
      ssymv_l_kernel_4x4(b, a0, a1, a2, a3, x, y, w0, w1);
    if (b < j)
      ssymv_l_kernel_1x4(b, j, a0, a1, a2, a3, x, y, w0, w1);

    // half block
    for (auto k = 0; k < 4; k++) {
      auto aj = get_row(m, a, j + k);
  ...
  for (auto j = (m / 4) * 4; j < m; j++) {
    ...
    auto a0 = get_row(m, a, j);

    auto b = (j / 8) * 8;

    // full block (4n x 4 and 4 x 4)
    if (b)
      ssymv_l_kernel_8x1(b, a0, x, y, &w0, &w1);
